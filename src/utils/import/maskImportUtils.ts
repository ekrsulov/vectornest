...
export interface MaskImportDefinition {
  id: string;
  name?: string;
  x?: string;
  y?: string;
 ...
 */
export const parseMaskDefs = (doc: Document): MaskImportDefinition[] => {
...
export const ensureMaskImports = (
  ...
  const existing = (pluginImports.mask as MaskImportDefinition[] | undefined) ?? [];
  const existingIds = new Set(existing.map((m) => m.id));
  const missing = Array.from(neededMaskIds).filter((id) => !existingIds.has(id));
  if (!missing.length) return pluginImports;

  const discovered: Array<MaskImportDefinition | null> = missing.map((id) => {
  ...
  const validMasks = discovered.filter((m): m is MaskImportDefinition => Boolean(m));
