    ...
    <Panel title="White Space" isCollapsible defaultOpen={false}>
      ...
      {m && (
        <>
          ...
          <StatRow label="Canvas Area" value={`${m.canvasArea.toLocaleString()} px²`} />
          <StatRow label="Elements Area" value={`${m.elementsArea.toLocaleString()} px²`} />
          <StatRow label="White Space" value={`${m.whiteSpaceArea.toLocaleString()} px²`} />
          ...
          <Box px={2} py={1}>
            ...
            <HStack justify="space-between" mt={0.5}>
              ...
              <Text fontSize="xs" color="gray.600" _dark={{ color: 'gray.400' }}>Empty {m.whiteSpacePercent}%</Text>
            </HStack>
          </Box>

          <SectionHeader title="Metrics" />
          <StatRow label="Utilization" value={`${m.utilization}%`} color={utilColor} />
          <StatRow label="Density" value={`${m.density} el/10k px²`} />
