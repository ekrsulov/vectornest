/**
 * AnimationCard — Shared collapsible card for editing a single SVG animation.
 *
 * Renders a compact summary when collapsed and a full property editor when expanded.
 * Supports all SMIL animation types: animate, animateTransform, animateMotion, set, custom.
 * Includes dynamic field visibility, spline-mode KeyframeEditor, and exhaustive
 * attribute options per animation type.
 *
 * Extracted from svgStructure to src/ui/ for reuse across plugins.
 */
...
import { Trash2, Move, RefreshCw, Eye, ChevronDown, ChevronRight, Plus, Zap, Settings } from 'lucide-react';
...
import { getAttributeOptionsForType, getTransformTypeOptions } from './animationAttributeOptions';
...
export interface AnimationCardProps {
    ...
    /** Canvas elements for motion path reference selector (animateMotion) */
...
export const AnimationCard: React.FC<AnimationCardProps> = ({ animation, onDelete, onUpdate, targetLabel, elements = [] }) => {
    ...
    // Standard types that show dynamic fields
    const isStandardType = animation.type === 'animate' || animation.type === 'animateTransform' || animation.type === 'set';
    ...
    // Attribute options based on animation type
    const attributeOptions = useMemo(
        () => getAttributeOptionsForType(animation.type),
        [animation.type],
    );

    // Transform type options
    const transformTypeOptions = useMemo(() => getTransformTypeOptions(), []);
            ...
            <VStack spacing={0} align="stretch">
                ...
                <Collapse in={isExpanded} animateOpacity>
                    <VStack spacing={2} align="stretch" px={1} pt={0} pb={2}>
                        {/* Target attribute — with exhaustive options per type */}
                        ...
                        {visibleFields.map(field => {
                            // If calcMode is spline, hide raw keyTimes/keySplines/values (KeyframeEditor handles them)
